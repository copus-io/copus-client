...
import { deleteLinkReq } from 'src/api/space/piece';
...
import useExternalLinksReq from 'src/data/use-space-external-link-list';
import AddLinkModal from './AddLinkModal';

const ExternalLinks = ({ spaceId }: { spaceId: string }) => {
  ...
  /** 接口-额外链接 */
  const { data = [], isLoading, mutate } = useExternalLinksReq(spaceId);
  /** 关闭弹框 */
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          setDelLoading(true);
          const res = await deleteLinkReq(id, spaceId);
          if (res.data.status) {
